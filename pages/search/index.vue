		...
		<view v-if="isHistory" class="history-box">
			<view v-if="historyList.length > 0">
				<view class="history-title">
					<text>搜索历史</text>
						<BaseIcon fontsize="40upx" icon="shanchu" @click="clearSearch" />
				</view>
				<view class="history-content">
					<view class="history-item" v-for="(item, index) in historyList" @click="listTap(item)" :key="index">
						{{ item.vod_name }}
					</view>
				</view>
			</view>
		...
		<view v-else class="history-box">
			<view v-if="historyList.length > 0" class="history-list-box">
				<view
					...
					@click="listTap(item)"
				>
					<rich-text :nodes="item.vod_name"></rich-text>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		async getInputtips(val) {
			let _this = this;
			const res = await this.$http.post('movie/dyController/getSearch', {
				name: val
			})
